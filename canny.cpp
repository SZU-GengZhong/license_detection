...
int_vec* create_int_vec(int size)
{
	...
	if (!iv) return iv;
	iv->data = (float*)calloc(1, size * sizeof(int));
	if (!iv->data)
	{
		destroy_int_vec(&iv);
		return NULL;
...
void stretching(real_2d* img) {
	float maxi=0, mini=100000;
	int w = img->cols;
	int h = img->rows;
	float** src = img->arr;
	for (int i = 0; i < h; i++) {
		for (int j = 0; j < w; j++) {
			maxi = src[i][j] > maxi ? src[i][j] : maxi;
			mini = src[i][j] < mini ? src[i][j] : mini;
		}
	}
	for (int i = 0; i < h; i++) {
		for (int j = 0; j < w; j++) {
			src[i][j] = 255 / (maxi - mini) * src[i][j] - (255 * mini) / (maxi - mini);
		}
	}
}
void binarization(real_2d* img) {
	float maxi = 0, mini = 100000;
	int w = img->cols;
	int h = img->rows;
	float** src = img->arr;
	...
	int x = maxi - ((maxi - mini) / 2);
	for (int i = 0; i < h; i++) {
		for (int j = 0; j < w; j++) {
			src[i][j] =  src[i][j] >= x ? 255 : 0;
		}
	}
...
void absiff(real_2d* img, uint8_t* t, int height,int width) {
	float** data = img->arr;
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			t[i * width + j] = abs(data[i][j] - t[i * width +j]);
		}
	}
}
